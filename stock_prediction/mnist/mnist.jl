...
entropy_summary = histogram_summary("Cross Entropy", cross_entropy)
merged_summary_op = merge_all_summaries()
summary_writer = train.SummaryWriter("./log")
...
for i in 1:200
  ...
  summaries = run(session, merged_summary_op, Dict(x=>batch[1], y_=>batch[2], keep_prob=>.5))
